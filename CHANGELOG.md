...
- **版本号与更新检查**：菜单 Tools > NonToon 亮度控制 > 关于与更新检查… 显示当前版本与索引里的最新版本，
  并可一键「检查更新 / 打开发布页」；另有 检查更新 菜单；窗口顶部也显示版本号与按钮；
  编辑器启动后每天自动检查一次（失败静默）。
...
- **某些材质的亮度「调不动」**：生效范围以前是「直接读共享遮罩的 A 通道，没设遮罩时才全生效」。
  但 NonToon 的 `_SharedMask` 是**所有模块共用**的一张 RGBA 贴图，转换插件会把描边 / 高光 /
  材质捕获这些遮罩烘进各自的通道；一旦有模块用了 A 通道，别的材质（例如身体）在 A 通道是黑色的部位
  就会 `llMask = 0` —— 表现就是「这块材质怎么调亮度都没反应」，把共享遮罩删掉又恢复正常。

  现在生效范围改成**显式开关**：

  ```
  _UseSharedMask（默认关闭）  关闭 = 整块材质都生效
  _GlobalMaskChannel         勾上以后用共享遮罩的哪个通道（R/G/B/A）
  ```

  默认关闭意味着：转换插件烘出来的遮罩留着也没关系，亮度依旧整块生效；想要「只在某个范围内提亮」
  再自己勾上并选通道。
