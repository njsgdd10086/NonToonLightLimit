...
| 遮罩范围（可选） | 勾上 **Mask Range** 后，用共享遮罩的某个通道控制生效范围，比如只提亮脸部 |
...
| **Mask Range** | 关 | 只在共享遮罩指定通道非黑的范围内生效（默认关闭 = 整块材质都生效） |
| **Mask Channel** | A | 勾上 **Mask Range** 以后，用共享遮罩的哪个通道限制生效范围 |
...
* 只想让一部分生效（比如不动脸），勾上 **Mask Range** 再选 **Mask Channel**；
* 想让某些材质完全不受影响：把它从动画里去掉，或者勾上 **Mask Range** 并把通道指向一个全 0 的通道。

> **为什么不默认用遮罩限制范围**：NonToon 的 `_SharedMask` 是所有模块共用的一张 RGBA 贴图，
> 转换插件会把描边 / 高光 / 材质捕获这些遮罩烘进各自的通道。如果亮度模块默认就去读某个通道，
> 那些通道在材质某些部位是黑的，就会表现成「这块材质怎么调亮度都没反应」。所以范围限制改成显式开关。
